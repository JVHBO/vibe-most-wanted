...
export default function TCGPage() {
  ...
  useEffect(() => {
    const initFarcasterWallet = async () => {
      ...
      try {
        // Check if we're actually in an iframe (miniapp context)
        ...
        // Check if we're in Farcaster context (must be in iframe AND have SDK)
        if (isInIframe && sdk && typeof sdk.wallet !== 'undefined' && sdk.wallet.ethProvider) {
          console.log('[Farcaster] ✅ Farcaster SDK detected in iframe, setting isInFarcaster=true');
  ...
  return (
    <div className="min-h-screen game-background text-vintage-ice p-4 lg:p-6 overflow-x-hidden relative">
      ...
      <header className={`flex flex-col items-center ${isInFarcaster ? 'gap-2 mb-2 p-2' : 'gap-3 md:gap-6 mb-4 md:mb-8 p-3 md:p-6'} bg-vintage-deep-black border-2 border-vintage-gold rounded-lg shadow-[0_0_30px_rgba(255,215,0,0.3)] ${isInFarcaster ? 'mt-[70px]' : ''}`}>
        {!isInFarcaster && (
