...
export function MissionsView({
  ...
  const toggleHowTo = (missionId: string) => {
    setExpandedHowTo(prev => prev === missionId ? null : missionId);
  };
      ...
      <div className="bg-vintage-charcoal/80 backdrop-blur-lg rounded-2xl border-2 border-vintage-gold/30 shadow-gold p-4 md:p-8">
        <div className="text-center mb-6">
          <h1 className="text-3xl md:text-4xl font-display font-bold text-vintage-gold mb-2">
            {"\u{1F381}"} {(t as (k: string) => string)('mPageTitle')}
          </h1>
          <p className="text-vintage-burnt-gold font-modern text-sm md:text-base">
            {(t as (k: string) => string)('mPageSubtitle')}
          </p>
        </div>
          ...
          <>
            <div className="space-y-4 mb-6">
              {missions.map((mission) => {
                ...
                const isHowToOpen = expandedHowTo === mission._id;
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <NextImage src={missionInfo.icon} alt="" width={28} height={28} />
                          <h3 className="text-base md:text-xl font-display font-bold text-vintage-gold leading-tight">
                            {(t as (k: string) => string)(missionInfo.titleKey)}
                          </h3>
                        </div>
                        <p className="text-sm text-vintage-burnt-gold font-modern mb-2">
                          {(t as (k: string) => string)(missionInfo.descKey)}
                        </p>

                        {/* How To toggle */}
                        <button
                          onClick={() => toggleHowTo(mission._id)}
                          className="text-xs text-vintage-gold/60 hover:text-vintage-gold border border-vintage-gold/20 hover:border-vintage-gold/50 rounded px-2 py-0.5 transition-all"
                        >
                          {isHowToOpen ? '▲ ' : '▼ '}{(t as (k: string) => string)('questHowTo')}
                        </button>

                        {/* How To content */}
                        {isHowToOpen && (
                          <p className="mt-2 text-xs text-vintage-gold/80 bg-vintage-gold/5 border border-vintage-gold/20 rounded-lg px-3 py-2 leading-relaxed">
                            {(t as (k: string) => string)(missionInfo.howToKey)}
                          </p>
                        )}
                      ...
                      <div className="flex-shrink-0">
                        {isClaimed ? (
                          ...
                          >
                            {isClaimingMission === mission._id ? (t as (k: string) => string)('mClaiming') : (t as (k: string) => string)('mClaim')}
                          </button>
                        ) : (
            ...
            {missions.some((m) => m.completed && !m.claimed) && (
              <div className="border-t-2 border-vintage-gold/20 pt-6">
                ...
                >
                  {isClaimingAll
                    ? (t as (k: string) => string)('mClaimingAll')
                    : `\u{1F381} ${(t as (k: string) => string)('mClaimAll')} (${missions.filter((m) => m.completed && !m.claimed).length})`}
                </button>
              </div>
            )}
          </>
