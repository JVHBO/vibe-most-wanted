...
export default function SlotReplay({ spins, totalWin, username, winType, amount, multX }: Props) {
  const [phase, setPhase] = useState<'spinning' | 'reveal' | 'done'>(spins.length ? 'spinning' : 'done');
      ...
      <div className="slotShell">
        ...
        {phase === 'done' && (
          <div className="resultPanel">
            ...
            <div className="resultAmount">+{shownWin.toLocaleString()}</div>
            {multX >= 2 && <div className="resultMult">{multX}x</div>}
            <a className="exitButton" href="/slot">Exit Replay</a>
          </div>
        )}
      </div>
