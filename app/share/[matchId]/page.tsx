...
export async function generateMetadata({ params }: { params: Promise<{ matchId: string }> }): Promise<Metadata> {
  ...
  // Decode match data from matchId (format: result_playerPower_opponentPower_opponentName_playerPfp_opponentPfp_playerName)
  ...
  // Determine type from lastBattleResult or default to pve
  // For now, detect type from opponent name
  let type = 'pve';
  if (opponentName.toLowerCase().includes('mecha')) {
    type = 'pve';
  } else {
    // If it's not Mecha, it's likely attack/pvp - default to attack
    type = 'attack';
  }
  ...
  const imageUrl = `${baseUrl}/share/${matchId}/opengraph-image?v=10`;
