...
/**
 * @title VBMSPokerBattleV5
 * @notice Gerencia poker battles com stakes em $VBMS
 * @dev V5 Improvements:
 *  - Removed activeBattles check in createBattle and joinBattle
 *  - Players can create new battles without calling finishBattle TX
 *  - Backend Convex validates "one active battle per player" instead
 *  - All other features from V4 remain (emergency functions, withdraw, etc)
 */
contract VBMSPokerBattleV5 is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public activeBattles; // Player => battleId (tracking only, not enforced)
     ...
     * @dev V5: Removed activeBattles check - validation done in backend Convex
    ...
    function createBattle(uint256 stake) external nonReentrant returns (uint256) {
        ...
        // ✅ V5: Removed activeBattles check - backend validates "one active battle per player"
        ...
        activeBattles[msg.sender] = battleId; // Still tracked for stats/view
