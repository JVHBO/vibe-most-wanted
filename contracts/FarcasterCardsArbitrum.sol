...
/// @title FarcasterCards (Arbitrum)
/// @notice Minimal ERC-721 contract for Farcaster playing cards with presigned minting
/// @dev Same as FarcasterCards on Base but with 25% discount on mint price
contract FarcasterCardsArbitrum is ERC721, EIP712, Ownable {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                         CONSTANTS                          */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/

    /// @dev Mint price in wei (0.000225 ETH - 25% discount from Base price)
