# 💰 SISTEMA DE ECONOMIA - $TESTVBMS
## Vibe Most Wanted - Complete Economy Guide

**Total Pool:** 10,000,000 $TESTVBMS

---

## 🎮 PvE REWARDS (Player vs AI)

| Difficulty | Coins/Win | Max Power | Daily Win Limit | Entry Fee | Daily Cap |
|-----------|-----------|-----------|-----------------|-----------|-----------|
| **Gey** | 5 | 200 | 30 | 🆓 FREE | 3,500 |
| **Goofy** | 15 | 400 | 30 | 🆓 FREE | 3,500 |
| **Gooner** | 30 | 600 | 30 | 🆓 FREE | 3,500 |
| **Gangster** | 60 | 800 | 30 | 🆓 FREE | 3,500 |
| **Gigachad** | 120 | 1,000 | 30 | 🆓 FREE | 3,500 |

**Notes:**
- PvE is the only mode with a daily coin cap (3,500 coins max)
- Win limit: 30 victories per day
- Losses don't count against the limit
- Best farming strategy: Gigachad difficulty (120 coins × 29 wins = 3,480 coins)

---

## ⚔️ PvP MODES (Player vs Player)

### Ranked PvP (Auto-Match)

| Feature | Value |
|---------|-------|
| **Entry Fee** | 20 coins |
| **Win Reward** | 100 coins base |
| **Loss Penalty** | -20 coins |
| **Daily Limit** | 10 matches |
| **Daily Cap** | ❌ NONE |
| **Ranking Bonus** | ✅ YES (see below) |

**Net Earnings (similar rank opponents):**
- Win: +80 coins (100 - 20 entry)
- Loss: -40 coins (-20 - 20 entry)

### Casual PvP (Custom Rooms)

| Feature | Value |
|---------|-------|
| **Entry Fee** | 🆓 FREE |
| **Win Reward** | 0 coins |
| **Loss Penalty** | 0 coins |
| **Daily Limit** | ∞ Unlimited |
| **Daily Cap** | ❌ NONE |
| **Ranking Bonus** | ❌ NO |

**Purpose:** Practice and fun without risk

---

## 🎯 ATTACK MODE (Leaderboard)

| Feature | Value |
|---------|-------|
| **Entry Fee** | 🆓 FREE |
| **Win Reward** | 100 coins base |
| **Loss Penalty** | -20 coins |
| **Daily Limit** | 5 attacks |
| **Daily Cap** | ❌ NONE |
| **Ranking Bonus** | ✅ YES (see below) |

**Best Strategy:** Attack top-ranked players for maximum bonuses!

---

## 📈 RANKING BONUS SYSTEM

### Win Multipliers (Based on Rank Difference)

| Rank Difference | Multiplier | Base Win | Final Win |
|----------------|------------|----------|-----------|
| **50+ ranks higher** | 2.0x | 100 | **200 coins** |
| **20-49 ranks higher** | 1.5x | 100 | **150 coins** |
| **10-19 ranks higher** | 1.3x | 100 | **130 coins** |
| **5-9 ranks higher** | 1.15x | 100 | **115 coins** |
| **< 5 ranks** | 1.0x | 100 | **100 coins** |

### Loss Penalty Reduction

| Rank Difference | Reduction | Base Loss | Final Loss |
|----------------|-----------|-----------|------------|
| **50+ ranks higher** | 60% | -20 | **-8 coins** |
| **20-49 ranks higher** | 50% | -20 | **-10 coins** |
| **10-19 ranks higher** | 35% | -20 | **-13 coins** |
| **5-9 ranks higher** | 20% | -20 | **-16 coins** |
| **< 5 ranks** | 0% | -20 | **-20 coins** |

**How it works:** Rank difference = Your Rank - Opponent Rank
- Example: You're rank #50, opponent is rank #3 → Difference = 47 ranks
- If you win: 100 × 2.0 = 200 coins
- If you lose: -20 × 0.4 = -8 coins

---

## 🎁 DAILY BONUSES

| Bonus | Coins | Condition |
|-------|-------|-----------|
| 🆕 **First PvE Win** | 50 | First PvE victory of the day |
| ⚔️ **First PvP Match** | 100 | First PvP match of the day (win or lose) |
| 📅 **Login Bonus** | 25 | Login each day |
| 🔥 **3-Win Streak** | 150 | Win 3 matches in a row (any mode) |
| 🔥 **5-Win Streak** | 300 | Win 5 matches in a row (any mode) |
| 🔥 **10-Win Streak** | 750 | Win 10 matches in a row (any mode) |

**Total Possible Daily Bonuses:** 1,425 coins (if all achieved)

---

## 🏆 WEEKLY REWARDS (Automated Distribution)

**Distribution Time:** Every Sunday at 00:00 UTC (automated via cron job)

| Rank Range | Reward | Requirements |
|-----------|--------|--------------|
| 🥇 **1st Place** | 1,000 coins | Top leaderboard position |
| 🥈 **2nd Place** | 750 coins | 2nd on leaderboard |
| 🥉 **3rd Place** | 500 coins | 3rd on leaderboard |
| ⭐ **4th-10th** | 300 coins | Top 10 position |
| ⭐ **11th-20th** | 150 coins | Top 20 position |
| ⭐ **21st-50th** | 75 coins | Top 50 position |

**How It Works:**
- Rewards are **automatically distributed** every Sunday at midnight UTC
- Based on your **leaderboard rank** at distribution time
- Coins are **instantly added** to your balance
- No action required - just maintain your rank!
- View countdown and your projected reward in the **Missions** tab

**Weekly Rewards UI:**
- ⏰ Real-time countdown to next distribution
- 📊 Your current rank and tier
- 💰 Your projected reward amount
- 🏅 TOP 10 leaderboard preview
- 🎯 Live position tracking

**Total Weekly Pool:** Up to 6,225 coins distributed to top 50 players

**Strategy Tips:**
- Grind on Saturday to secure your rank
- Monitor the TOP 10 preview to see competition
- Even TOP 50 earns rewards - stay active!
- Rewards stack with daily earnings

---

## 🎯 ACHIEVEMENT SYSTEM

**Total Available:** 64 achievements, ~302,000 coins

### Achievement Categories

#### 💎 Rarity Collectors (6 achievements)
Unlock by owning specific rarity cards for the first time.

| Achievement | Requirement | Reward |
|------------|-------------|--------|
| Common Collector | Own 1 Common | 50 coins |
| Uncommon Collector | Own 1 Uncommon | 75 coins |
| Rare Collector | Own 1 Rare | 100 coins |
| Epic Collector | Own 1 Epic | 200 coins |
| Legendary Collector | Own 1 Legendary | 500 coins |
| Mythic Collector | Own 1 Mythic | 1,000 coins |

#### ✨ Wear Collectors (10 achievements)
Collect cards in pristine condition.

| Achievement | Requirement | Reward |
|------------|-------------|--------|
| Pristine Collector | Own 1 Pristine | 300 coins |
| Mint Collector | Own 1 Mint | 250 coins |
| Pristine Hoarder | Own 10 Pristine | 1,000 coins |
| Pristine Master | Own 50 Pristine | 5,000 coins |
| Pristine Legend | Own 100 Pristine | 15,000 coins |
| ... | ... | ... |

#### 🎴 Foil Collectors (9 achievements)
Hunt for shiny and prize foil variants.

| Achievement | Requirement | Reward |
|------------|-------------|--------|
| Shiny Collector | Own 1 Standard Foil | 200 coins |
| Prize Winner | Own 1 Prize Foil | 500 coins |
| Foil Enthusiast | Own 10 Standard Foils | 1,000 coins |
| Elite Collector | Own 10 Prize Foils | 2,000 coins |
| Prize Legend | Own 50 Prize Foils | 10,000 coins |
| ... | ... | ... |

#### 📊 Progressive Challenges (39 achievements)
Milestone-based collection goals across all rarities.

**Examples:**
- Common Beginner I-VI (1/5/10/25/50/100 Commons) - 10 to 1,000 coins each
- Rare Beginner I-VI (1/5/10/25/50/100 Rares) - 50 to 5,000 coins each
- Epic Beginner I-VI (1/5/10/25/50/100 Epics) - 100 to 10,000 coins each
- Legendary series - 200 to 25,000 coins each
- Mythic God (100 Mythics) - 50,000 coins 🏆

**Achievement Features:**
- ✅ Auto-detection when you collect new NFTs
- ✅ Progress tracking for all 64 achievements
- ✅ One-click claim for completed achievements
- ✅ Claim all unclaimed rewards at once
- ✅ Stats dashboard showing completion percentage
- ✅ Filter by category (Rarity, Wear, Foil, Progressive)

**Access:** Go to ★ **Achievements** tab in main navigation

---

## 💡 EXAMPLE SCENARIOS

### Scenario 1: Rank #50 Attacks Rank #3 (Underdog Attack)
- **Entry Fee:** 0 (free attack)
- **Rank Difference:** 47 ranks
- **If WIN:** 100 × 2.0 = **+200 coins** 💰
- **If LOSE:** -20 × 0.4 = **-8 coins** 💀
- **Risk/Reward Ratio:** -8 / +200 (25:1 reward!)

### Scenario 2: Rank #10 Attacks Rank #15 (Similar Ranks)
- **Entry Fee:** 0 (free attack)
- **Rank Difference:** 5 ranks (no bonus threshold)
- **If WIN:** **+100 coins** 💰
- **If LOSE:** **-20 coins** 💀
- **Risk/Reward Ratio:** -20 / +100 (5:1 reward)

### Scenario 3: PvP Ranked Auto-Match (Similar Ranks)
- **Entry Fee:** -20 coins
- **Rank Difference:** < 5 ranks (typical)
- **If WIN:** 100 - 20 = **+80 coins net** 💰
- **If LOSE:** -20 - 20 = **-40 coins net** 💀
- **Risk/Reward Ratio:** -40 / +80 (2:1 reward)

### Scenario 4: Farm PvE Gigachad (30 Wins)
- **Entry Fee:** 0 (free)
- **30 wins × 120 coins:** 3,600 coins earned
- **Daily Cap Applied:** Limited to **3,500 coins**
- **Actual Earnings:** ~29 wins = 3,480 coins
- **Total Net:** **+3,500 coins** (cap reached)

### Scenario 5: Perfect Daily Grind
```
1. PvE Gigachad (29 wins):        +3,500 coins (capped)
2. Attack top 5 players (5 wins): +1,000 coins (5 × 200)
3. PvP Ranked (10 wins):          +800 coins (10 × 80 net)
4. Daily bonuses (all):           +1,425 coins
────────────────────────────────────────────────
TOTAL:                            +6,725 coins/day
```

---

## 📊 MAXIMUM EARNINGS

### Daily Earnings

| Source | Max Amount | Notes |
|--------|------------|-------|
| **PvE** | 3,500 | Daily cap at 3,500 coins |
| **Attack** | 1,000 | 5 attacks × 200 coins (vs top players) |
| **PvP Ranked** | 800 | 10 matches × 80 net (after entry fees) |
| **Daily Bonuses** | 1,425 | All bonuses achieved |
| **TOTAL DAILY** | **6,725** | Theoretical maximum per day |

### Weekly Earnings

| Source | Max Amount | Notes |
|--------|------------|-------|
| **Daily Grind** | 47,075 | 6,725 × 7 days |
| **Weekly Rewards** | 1,000 | If you're rank #1 |
| **Achievements** | Variable | One-time claims as you collect NFTs |
| **TOTAL WEEKLY** | **48,075+** | Without achievements |

### Achievement Potential

| Category | Total Coins | Notes |
|----------|------------|-------|
| **Rarity Collectors** | 1,925 | 6 achievements |
| **Wear Collectors** | 24,950 | 10 achievements |
| **Foil Collectors** | 23,950 | 9 achievements |
| **Progressive** | ~251,000 | 39 milestone achievements |
| **TOTAL** | **~302,000** | One-time collection rewards |

---

## 💸 COST BREAKDOWN

| Activity | Entry Cost | Max Daily Spend | Max Earnings | Net Profit |
|----------|-----------|----------------|--------------|------------|
| **PvE** (30 matches) | 0 | 0 | 3,500 | +3,500 |
| **Attack** (5 matches) | 0 | 0 | 1,000 | +1,000 |
| **PvP Ranked** (10 matches) | 20 each | 200 | 1,000 | +800 |
| **PvP Casual** (unlimited) | 0 | 0 | 0 | 0 |

---

## 🎯 OPTIMAL STRATEGIES

### 🌟 **Zero-Cost Strategy** (Risk-Free)
- Focus on **PvE** (free, 3,500 cap)
- Use all 5 **Attack** slots (free, target top players)
- Avoid PvP Ranked (costs 20 per match)
- Use **Casual PvP** to practice

**Expected Daily:** 4,000-4,500 coins

---

### 💪 **Aggressive Strategy** (High Risk/Reward)
- Farm **PvE Gigachad** to 3,500 cap
- Attack **top 5 players** only (maximize bonuses)
- Play **10 PvP Ranked** matches (must maintain 50%+ winrate)
- Get all **daily bonuses** and **streaks**

**Expected Daily:** 6,000-6,725 coins

---

### 🧠 **Smart Strategy** (Balanced)
- Farm **PvE** to 2,000-3,000 coins
- Use **Attack** on players 10-20 ranks above
- Play **5 PvP Ranked** when confident
- Always claim **First PvE** and **Login** bonuses

**Expected Daily:** 3,500-5,000 coins

---

## 📉 LOSING SCENARIOS (What to Avoid)

### ❌ Bad: Rank #3 Attacks Rank #50
- Entry: 0
- Rank Difference: -47 (attacking down)
- Win: +100 coins (no bonus for attacking lower)
- Loss: **-20 coins** (full penalty)
- **Not worth it!**

### ❌ Bad: PvP Ranked with Low Winrate
- Entry: -20 per match
- 10 matches with 30% winrate:
  - 3 wins: +240 coins
  - 7 losses: -140 coins
  - Entry fees: -200 coins
  - **Net: -100 coins** (losing money!)

---

## 📋 DAILY LIMITS REFERENCE

| Mode | Entry Fee | Limit Type | Limit | Daily Cap |
|------|-----------|-----------|--------|-----------|
| **PvE** | 🆓 FREE | Wins | 30 | 3,500 coins |
| **PvP Ranked** | 20 coins | Matches | 10 | ❌ NONE |
| **PvP Casual** | 🆓 FREE | Matches | ∞ | ❌ NONE |
| **Attack** | 🆓 FREE | Attacks | 5 | ❌ NONE |

---

## 📜 CHANGELOG

| Date | Change | Old | New | Reason |
|------|--------|-----|-----|--------|
| 2025-11-03 | **Weekly Rewards** | N/A | Automated (cron) | Reward top 50 players |
| 2025-11-03 | **Achievement System** | N/A | 64 achievements | ~302K coins available |
| 2025-11-03 | **UI Consistency** | Mixed themes | Vintage casino gold | Consistent branding |
| 2025-11-03 | **i18n Support** | English only | 4 languages | Global accessibility |
| 2025-01 | Attack Entry Fee | 50 coins | 🆓 FREE | Remove barrier for competition |
| 2025-01 | PvP Entry Fee | 40 coins | 20 coins | Make PvP more accessible |
| 2025-01 | Attack Daily Cap | 3,500 | ❌ NONE | 5 attacks/day is enough |
| 2025-01 | PvP Daily Cap | 3,500 | ❌ NONE | 10 matches/day is enough |
| 2025-01 | Casual PvP | N/A | Added | Free practice mode |

---

## ⚠️ IMPORTANT NOTES

1. **PvE Daily Cap (3,500)** applies ONLY to PvE earnings, not total daily earnings
2. **Ranking bonuses** calculate from rank difference, not absolute position
3. **Entry fees** are paid BEFORE knowing the match outcome
4. **Daily limits** reset at midnight UTC
5. **Streaks** carry over between different game modes
6. **Casual PvP** is perfect for testing decks without risk
7. **Attack mode** is completely free - use all 5 daily attacks!
8. **Weekly Rewards** are automatically distributed every Sunday 00:00 UTC
9. **Achievements** auto-detect when you collect new NFTs - check the ★ Achievements tab!
10. **Test Page** (`/test`) available for development without wallet connection

---

**Generated:** 2025-11-03
**Version:** 3.0 (Achievement & Weekly Rewards Update)
**Status:** ✅ Active
