...
# Add bet buttons after opponent hand section (around line 1690)
# Find the closing div of opponent hand
bet_buttons_html = """
...
# Looking for the pattern after opponentHand.map
insert_pattern = """                  </div>
                </div>

                {/* CENTER - CARD BATTLE AREA */}"""

content = content.replace(insert_pattern, f"""                  </div>
{bet_buttons_html}
                </div>
...
content = content.replace(player_pattern, f"""                  </div>
{player_bet_buttons}
                </div>
...
print("Added spectator bet buttons!")
