...
export const spinSlot = mutation({
  ...
  handler: async (
    ...
    const baseComboChance = isBonusMode ? 0.75 : (
      totalSpinsToday < 3  ? 0.99 :
      totalSpinsToday < 6  ? 0.92 :
      totalSpinsToday < 10 ? 0.82 :
      totalSpinsToday < 20 ? 0.72 : 0.65
    );
    const pityBoost = isBonusMode ? 0 : Math.min(0.5, noComboStreak * 0.25);
    const comboBoostChance = Math.min(0.99, baseComboChance + pityBoost);
    const forceComboNow = !isBonusMode && noComboStreak >= 2;
